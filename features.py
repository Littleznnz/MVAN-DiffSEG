...
def process_data(path, t=2, overlap=1, RATE=16000):
    path = path.rstrip('/')
...
class FeatureExtractor(object):
    def __init__(self, rate,spec = False):
        self.rate = rate
        self.sepc_flag = spec
    ...
    def get_mfcc(self, X, n_mfcc=13):
        spec_flag = self.sepc_flag
        def _get_mfcc(x):
            # spec = librosa.feature.pec
            # mel = librosa.feature.melspectrogram(y=x,sr=self.rate)
            # mel = np.expand_dims(mel,0)
            # mel_p = torch.Tensor(mel)
            # spec_augment_pytorch.spec_augment(mel_spectrogram=mel_p,time_warping_para=5)
            # # S = librosa.power_to_db(warpd_mel)
            ...
            if spec_flag == True:
                ...
                mfcc_data = librosa.feature.mfcc(y=x, sr=self.rate, n_mfcc=n_mfcc)
                mfcc_spec = torch.Tensor(mfcc_data).unsqueeze(0)
                mfcc_data = spec_augment_pytorch.spec_augment(mel_spectrogram=mfcc_spec,
                                                              time_warping_para=5,
                                                              frequency_masking_para=7,
                                                              time_masking_para=15).squeeze(0)
            ...
            # cax = ax.imshow(delta, interpolation='nearest', cmap=cm.coolwarm, origin='lower')
            # ax.set_title('DELTA')
            # plt.show()
            # fig, ax = plt.subplots()
            # delta_delta = np.swapaxes(delta_delta, 0, 1)
            # cax = ax.imshow(delta_delta, interpolation='nearest', cmap=cm.coolwarm, origin='lower')
            ...
            # out = np.concatenate((delta, delta_delta), 0)
            return out
            # return mfcc_data
...
if __name__ == '__main__':
    # X, y = process_data('D:/IEMOCAP_full_release/data')
    X, y = process_data(r'E:/pytorch_project/speech_emotion_recognition/interspeech2023/interspeech21_emotion/path_to_wavs')
    n = len(X)
